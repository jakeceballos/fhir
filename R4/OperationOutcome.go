...
func (strct *OperationOutcome) MarshalJSON() ([]byte, error) {
    ...
    // Marshal the "issue" field
    if comma { 
        buf.WriteString(",") 
    }
    buf.WriteString("\"issue\": ")
	if tmp, err := json.Marshal(strct.Issue); err != nil {
		return nil, err
 	} else {
 		buf.Write(tmp)
	}
...
func (strct *OperationOutcome) UnmarshalJSON(b []byte) error {
    ...
    for k, v := range jsonMap {
        ...
        case "issue":
            if err := json.Unmarshal([]byte(v), &strct.Issue); err != nil {
                return err
             }
            issueReceived = true
        case "language":
    ...
    // check if issue (a required property) was received
    if !issueReceived {
        return errors.New("\"issue\" is required but was not present")
    }
