...
func (strct *ContractSecurityLabel) MarshalJSON() ([]byte, error) {
	...
	comma = true
    // "Classification" field is required
    if strct.Classification == nil {
        return nil, errors.New("classification is a required field")
    }
	...
	comma = true
    // Marshal the "control" field
    if comma { 
        buf.WriteString(",") 
    }
    buf.WriteString("\"control\": ")
	if tmp, err := json.Marshal(strct.Control); err != nil {
		return nil, err
 	} else {
 		buf.Write(tmp)
	}
...
func (strct *ContractSecurityLabel) UnmarshalJSON(b []byte) error {
    ...
    for k, v := range jsonMap {
        ...
        case "control":
            if err := json.Unmarshal([]byte(v), &strct.Control); err != nil {
                return err
             }
        case "extension":
    ...
    // check if classification (a required property) was received
    if !classificationReceived {
        return errors.New("\"classification\" is required but was not present")
    }
    return nil
}
