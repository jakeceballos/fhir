...
type ResearchElementDefinition struct {
  ...
  VariableType interface{} `json:"variableType,omitempty"`
...
func (strct *ResearchElementDefinition) MarshalJSON() ([]byte, error) {
	...
	comma = true
    // "Characteristic" field is required
	...
	comma = true
    // Marshal the "variableType" field
    if comma { 
        buf.WriteString(",") 
    }
    buf.WriteString("\"variableType\": ")
	if tmp, err := json.Marshal(strct.VariableType); err != nil {
		return nil, err
 	} else {
 		buf.Write(tmp)
	}
...
func (strct *ResearchElementDefinition) UnmarshalJSON(b []byte) error {
    ...
    for k, v := range jsonMap {
        ...
        case "variableType":
            if err := json.Unmarshal([]byte(v), &strct.VariableType); err != nil {
                return err
             }
        case "version":
    ...
    // check if characteristic (a required property) was received
    if !characteristicReceived {
        return errors.New("\"characteristic\" is required but was not present")
    }
